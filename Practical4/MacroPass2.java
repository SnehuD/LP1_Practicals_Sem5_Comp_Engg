...
public class MacroPass2 {
	public static void main(String[] args) throws Exception {
		...
		while ((line = irb.readLine()) != null) // read intermediate code
		{
			String[] parts = line.split("\\s+");
			if (mnt.containsKey(parts[0])) // check macro name in Mnt table if found then get all the field from mnt
											// table
			{
				...
				paramNo = 1;
				for (int i = 0; i < pp; i++) // read parameter of m1 macro from intermediate code
				{
				...
				int j = kpdtp - 1;
				for (int i = 0; i < kp; i++) // read the parameter from kpdt table
				{
				...
				int i = mdtp - 1;
				while (!mdt.get(i).equalsIgnoreCase("MEND")) // Read Mdt Table till "mend" found
				{
...
//not containing number
		...
		kpdtb.close();
		irb.close();
		System.out.println("macro pass 2 proceesing done:)");
	}
}
